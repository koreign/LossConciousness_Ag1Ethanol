...
%% Load data
dirSave = '';
load(strcat(dirSave,'SOOptoStim-StimLockedAvg_SpiTroughs-SlSpiEvts'));
...
arrYLim         = [0 15; 0 10; 0 4];                                % array setting the y-limits for each channel
...
for iCh = 1 : 3         % loop across FRO, PAR and CA1 channels
   for iComp = 1 : 3    % loop across 3 comparisons (IN vs. OUT, IN vs. NoStim-IN and OUT vs. NoStim-OUT) 
        ...
        ax1 = subplot(5,1,1:2);
        hold on;
        trace1SEM   = area(resStimSpiTrAvg{arrComp(iComp,1),iCh}.time(1:end-1,1),...
                           (resStimSpiTrAvg{arrComp(iComp,1),iCh}.grandavgMean + resStimSpiTrAvg{arrComp(iComp,1),iCh}.grandavgSEM)*100,...
                           'FaceColor',arrColorSEM{iComp,1},'EdgeColor',arrColorSEM{iComp,1});
        trace1Mean  = area(resStimSpiTrAvg{arrComp(iComp,1),iCh}.time(1:end-1,1), resStimSpiTrAvg{arrComp(iComp,1),iCh}.grandavgMean*100,'FaceColor',arrColorMean{iComp,1},'EdgeColor',arrColorMean{iComp,1});
...
%         set(gca,'YTickLabel',arrYTicks{iCh,2},'FontSize',8);
        ...
        %title({' ';'test '})
        hold on;
        trace2SEM   = area(resStimSpiTrAvg{arrComp(iComp,2),iCh}.time(1:end-1,1),...
                           (resStimSpiTrAvg{arrComp(iComp,2),iCh}.grandavgMean + resStimSpiTrAvg{arrComp(iComp,2),iCh}.grandavgSEM)*100,...
                           'FaceColor',arrColorSEM{iComp,2},'EdgeColor',arrColorSEM{iComp,2});
        trace2Mean  = area(resStimSpiTrAvg{arrComp(iComp,2),iCh}.time(1:end-1,1), resStimSpiTrAvg{arrComp(iComp,2),iCh}.grandavgMean*100,'FaceColor',arrColorMean{iComp,2},'EdgeColor',arrColorMean{iComp,2});
...
%         set(gca,'YTickLabel',arrYTicks{iCh,2},'FontSize',8);
        ...
        stat = bar(resStimSpiTrAvg{1,1}.time(1:end-1,1),pVal,'BaseValue',0.05,'FaceColor',[0.5 0.5 0.5],'EdgeColor',[0.5 0.5 0.5]);
